...
class RealmanRobot:
    ...
    def __init__(
        ...
        *,
        ...
        self._video_path = self._run_dir / "observations_3hz.mp4"
        self._action_plot_path = self._run_dir / "actions_plot.png"
        ...
        for idx, act in enumerate(act_seq[:limit]):
            ...
            try:
                ...
                self._action_history.append(
                    {
                        "timestamp": row[0],
                        "action_global_idx": self._action_counter,
                        "obs_step": obs_step,
                        "values": cmd_list,
                    }
                )
    ...
    def _make_video(self, fps: int = 15) -> None:
        image_pairs = sorted(self._image_dir.glob("*_cam_high.png"))
        if not image_pairs:
            logging.warning("No images found in %s; skip video export.", self._image_dir)
            return

        # Prepare first frame to determine size
        first_high = cv2.imread(str(image_pairs[0]))
        if first_high is None:
            logging.warning("Failed to read first image %s; skip video export.", image_pairs[0])
            return
        first_token = image_pairs[0].name.split("_")[0]
        first_left_path = self._image_dir / f"{first_token}_cam_left_wrist.png"
        first_left = cv2.imread(str(first_left_path))
        if first_left is None:
            first_left = first_high.copy()
        if first_left.shape[:2] != first_high.shape[:2]:
            first_left = cv2.resize(first_left, (first_high.shape[1], first_high.shape[0]))
        frame_h, frame_w = first_high.shape[0], first_high.shape[1] + first_left.shape[1]
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        writer = cv2.VideoWriter(str(self._video_path), fourcc, fps, (frame_w, frame_h))

        written = 0
        for high_path in image_pairs:
            token = high_path.name.split("_")[0]
            left_path = self._image_dir / f"{token}_cam_left_wrist.png"
            img_high = cv2.imread(str(high_path))
            if img_high is None:
                logging.warning("Failed to read %s; skip.", high_path)
                continue
            img_left = cv2.imread(str(left_path))
            if img_left is None:
                img_left = img_high.copy()
            if img_left.shape[:2] != img_high.shape[:2]:
                img_left = cv2.resize(img_left, (img_high.shape[1], img_high.shape[0]))
            frame = np.hstack([img_high, img_left])
            writer.write(frame)
            written += 1
        writer.release()
        logging.info("Saved video %s with %s frames at %s fps.", self._video_path, written, fps)

    def _plot_actions(self) -> None:
        if not self._action_history:
            logging.warning("No actions recorded; skip plotting.")
            return
        timestamps = np.asarray([a["timestamp"] for a in self._action_history], dtype=np.float64)
        actions = np.asarray([a["values"] for a in self._action_history], dtype=np.float32)
        if actions.ndim != 2:
            logging.warning("Unexpected action history shape %s; skip plotting.", actions.shape)
            return
        rel_time = timestamps - timestamps[0]
        fig, ax = plt.subplots(figsize=(8, 4))
        for dim in range(actions.shape[1]):
            ax.plot(rel_time, actions[:, dim], label=f"joint_{dim}")
        ax.set_xlabel("time (s, relative)")
        ax.set_ylabel("action value")
        ax.set_title("Executed actions over time")
        ax.grid(True, linestyle="--", alpha=0.4)
        ax.legend(loc="upper right", ncol=2, fontsize=8)
        fig.tight_layout()
        fig.savefig(self._action_plot_path)
        plt.close(fig)
        logging.info("Saved action plot to %s", self._action_plot_path)

    def _export_artifacts(self) -> None:
        try:
            self._make_video(fps=3)
        except Exception:
            logging.exception("Failed to generate video at %s", self._video_path)
        try:
            self._plot_actions()
        except Exception:
            logging.exception("Failed to plot actions to %s", self._action_plot_path)

    def close(self) -> None:
        try:
            try:
                self._collector.stop_collection()
            except BaseException as exc:  # catch KeyboardInterrupt so artifacts still export
                logging.warning("stop_collection interrupted: %s", exc)
            try:
                self._collector.cleanup()
            except BaseException as exc:  # catch KeyboardInterrupt so artifacts still export
                logging.warning("cleanup interrupted: %s", exc)
        finally:
            try:
                self._export_artifacts()
            except Exception:
                logging.exception("Failed to export artifacts.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "/home/belrin/Berlin/lerobot/outputs/train/realman_data_single_peg_pose_20251210_v21/checkpoints/last/pretrained_model"
        ),
